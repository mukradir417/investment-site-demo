...
const ReviewTask = require('./models/ReviewTask'); // 🔥 New Model
...
// Database Connection
mongoose.connect('mongodb://127.0.0.1:27017/earning_pro_db')
.then(async () => {
...
cron.schedule('0 6 * * *', async () => {
    try {
        ...
        // 3. 🔥 RESET DAILY TASK COUNT (Start fresh every day)
...
// Admin: Set Manual Task Limit for User
...
// User: Get Random Task (With Limit Check)
...
// ============================================
// 🔥 FINANCIAL API (Deposit & Withdraw)
// ============================================

app.post('/deposit', async(req,res)=>{
    const u=await User.findById(req.body.userId);
    await new Deposit({userId:u._id,userName:u.name,amount:req.body.amount,trxId:req.body.trxId,method:req.body.method}).save();
    res.json({success:true,message:"Submitted"});
});

// ⛔ WITHDRAW (With Task Limit Check) ⛔
app.post('/withdraw', async(req, res) => {
    ...
    try {
        ...
        if (amount < 300) return res.json({ success: false, message: "Min withdraw 300 Tk" });
...
// ============================================
// 🔥 GENERAL API (Auth, Settings, Admin)
// ============================================
